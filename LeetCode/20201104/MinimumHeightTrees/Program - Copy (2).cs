...
namespace MinimumHeightTrees
{
    ...
    public class TreeNode
    {
        ...
        // If the subtree below is taller than an already found Minimum Height Tree
        //  -> no need to consider the tree that constitutes this subtree as it would anyway be a taller one
        ...
        public int CalculateDepth(int terminalDepth, ref bool candidate)
        {
            if (!Candidate)
            {
                // this node which is on the way is already taller -> no need to consider this tree
                candidate = false;
            ...
            if (!Depth.HasValue)
            {
                Depth = CalculateDepth(terminalDepth, new HashSet<int>() { Label }, ref candidate);
                Candidate = candidate;
            }
        ...
        private int CalculateDepth(int terminalDepth, HashSet<int> visited, ref bool candidate)
        {
            var maxDepth = 0;
            ...
            foreach (var child in Edges)
            {
                ...
                if (!child.Candidate)
                {
                    // this "child" node which is on the way is already taller
                    //  -> no need to consider the "tree" that constitutes this "child" as subtree
                    candidate = false;
                    return Int32.MaxValue;
                }

                var depthOfSubtree = child.Depth ?? child.CalculateDepth(terminalDepth, visited, ref candidate);

                if (!candidate)
                {
                    // this child which is on the way is already taller
                    //  -> no need to consider the tree that constitutes this subtree
                    return Int32.MaxValue;
                }

                if (maxDepth > terminalDepth)
                {
                    // at least one child path starts going beyond allowed terminalDepth
    ...
    public class Solution
    {
        public IEnumerable<TreeNode> FormulateTree(int n, int[][] edges)
        {
            ...
            for (int i = 0; i < edges.Length; i++)
            {
                if (edges[i].Any())
                {
                    var leftNode = tree[edges[i][0]];
                    var rightNode = tree[edges[i][1]];
        ...
        public IList<int> FindMinHeightTrees(int n, int[][] edges)
        {
            ...
            {
                bool candidate = true;
                root.Depth = root.CalculateDepth(minimumHeight, ref candidate);
                root.Candidate = candidate;

                // if (!candidate)
                //  -> the subtree below "root" is taller than already existing minimum height tree
                //  -> so, no need to consider the tree which has "root" as its root node
                //  -> "depth" is already returned as Int32.MaxValue, in this case

                if (candidate)
                {
                    if (minimumHeight == root.Depth)
                    {
                        // another tree with same minimum height found -> consider it
                        minimumHeightTreeRoots.Add(root.Label);
                    }
                    else if (minimumHeight > root.Depth)
                    {
                        // found a new minimum height -> discard already found minimum-height-trees and start considering new ones
                        minimumHeight = root.Depth.Value;
