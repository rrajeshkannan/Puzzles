...
Console.WriteLine(new Solution().DominantIndices([5, 4, 3])); // Output: 2
Console.WriteLine(new Solution().DominantIndices([4, 1, 2])); // Output: 1
Console.WriteLine(new Solution().DominantIndices([1])); // Output: 1
...
public class Solution
{
    public int DominantIndices(int[] nums)
    {
        ...
        for (int i = n - 2; i >= 0; i--)
        {
            double average = (double)suffixSum / (n - 1 - i);
            if (nums[i] > average)
            {
                count++;
            }
