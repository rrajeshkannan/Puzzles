...
//
// You must repeatedly apply the following merge operation until no more changes can be made: if any two adjacent elements are equal, choose the leftmost such adjacent pair in the current array and replace them with a single element equal to their sum.
...
Console.WriteLine(string.Join(",", new Solution().MergeAdjacent([3, 1, 1, 2]))); // [3,4]
Console.WriteLine(string.Join(",", new Solution().MergeAdjacent([2, 2, 4]))); // [8]
Console.WriteLine(string.Join(",", new Solution().MergeAdjacent([3, 7, 5]))); // [3,7,5]
...
public class Solution
{
    public IList<long> MergeAdjacent(int[] nums)
    {
        ...
        foreach (var num in nums.Skip(1))
        {
            if (stack.Peek() == num)
            {
                var sum = stack.Pop() + num;

                while (stack.TryPeek(out var peek) && peek == sum)
                {
                    sum += stack.Pop();
                }
